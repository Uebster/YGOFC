...
public class FusionManager : MonoBehaviour
{
    ...
    public bool CanBeFusionSummoned(CardData fusionMonster, List<CardData> availableCards)
    {
        List<string> requiredNames = new List<string>();
        if (fusionMonster.fusion_materials != null && fusionMonster.fusion_materials.Count > 0)
            requiredNames = new List<string>(fusionMonster.fusion_materials);
        else
            requiredNames = GetMaterialsFromDescription(fusionMonster.description);

        if (requiredNames.Count == 0) return false;

        var availableNames = availableCards.Select(c => c.name).ToList();
        int substituteCount = 0;
        ...
        foreach (string requiredName in requiredNames)
        {
            ...
            {
                availableNames.Remove(requiredName);
            }
            else
            {
                var sub = availableCards.FirstOrDefault(c => IsSubstitute(c) && availableNames.Contains(c.name));
                if (sub != null && substituteCount < 1) {
                    availableNames.Remove(sub.name);
                    substituteCount++;
                } else {
                    return false;
                }
    ...
    public void PerformFusionSummon(CardDisplay sourceCard, CardData fusionMonster, List<CardData> materials, bool isDefense)
    {
